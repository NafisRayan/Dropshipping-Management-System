...
export default async function DashboardPage() {
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-screen-2xl flex-col gap-6 px-4 py-6 sm:gap-7 sm:px-6 sm:py-8 lg:gap-9 lg:px-10">
      <header className="flex flex-col gap-5 rounded-3xl border border-border/60 bg-card/70 p-4 sm:gap-6 sm:p-5 md:flex-row md:items-center md:justify-between md:gap-8 md:p-6 lg:gap-9 lg:p-7">
      ...
      <div className="grid gap-4 sm:gap-6 lg:grid-cols-[2fr_1fr] lg:gap-8">
        <PerformanceChart data={data.monthlyPerformance} />
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1 lg:gap-6">
          <UserStatsCard stats={data.userStats} />
          <RoleDistribution stats={data.userStats} />
        </div>
      </div>

      <div className="grid gap-4 sm:gap-6 lg:grid-cols-[1.25fr_1fr] lg:gap-8 xl:grid-cols-[1.5fr_1fr]">
        <TopProducts products={data.topProducts} />
        <ChannelPerformanceCard data={data.channelPerformance} />
      </div>

      <div className="grid gap-4 sm:gap-6 lg:grid-cols-[1.5fr_1fr] lg:gap-8">
